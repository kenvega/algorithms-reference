...
const snail = (multiArray) => {
  ...
  for (let i = 0; i < iterationsNeeded; i++) {
    ...
    const rightElements = multiArray.map(
      (array) => array.splice(array.length - 1, 1)[0],
    )
    result = [...result, ...rightElements]

    const bottomElements = multiArray
      .splice(multiArray.length - 1, 1)[0]
      .reverse()
    result = [...result, ...bottomElements]

    const leftElements = multiArray
      .map((array) => array.splice(0, 1)[0])
      .reverse()
